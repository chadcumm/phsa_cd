/***********************************************************************************************************************
  Program Name:			bc_all_pex_warfarin_log_st
  Source File Name:		bc_all_pex_warfarin_log_st.prg
...
drop program bc_amb_st_anticoag_log:dba go
create program bc_amb_st_anticoag_log:dba
...
head report
	...
	call add_cell( "\b  Anti-Xa", "BOX", vAntiXa, 0, 0 )
...
if (qualify = 1)
 ...
 if (vRowCnt <= 10)
	dose_taken_vc = dose_taken
	dose_rec_vc = recommended_dose
	
	call clear_row(0, 14)
	call add_cell( concat(" ",result_val_dt), "BOX", vDate, 0, 1 )
	call add_cell( concat(" ",INR), "BOX", vINR, 0, 1 )
	call add_cell( concat(" ",AntiXa), "BOX", vAntiXa, 0, 1 )
	call add_cell( concat(" ",type), "BOX", vType, 0, 1 )
	call add_cell( concat(" ",dose_taken_vc), "BOX", vDoseTaken, 0, 1 )
	
	
	call add_cell( concat(" ",recommended_dose), "BOX", vRcmDose, 0, 1 )
	call add_cell( concat(" ",recommended_by), "BOX", vRcmBy, 0, 1 )
	;call add_cell( concat(" ",pt_contacted), "BOX", vPatCont, 0, 1 )
	call add_cell( concat(" ",contact_dt), "BOX", vContDate, 0, 1 )
	call add_cell( concat(" ",comm_method), "BOX", vCommMeth, 0, 1 )
	call add_cell( concat(" ",next_check), "BOX", vNextCheck, 0, 1 )
	call add_cell( concat(" ",notes), "BOX", vNotes, 0, 1 )
